...
import { generateProducts, datasetSizes, type Product } from "./shared_datasets.ts";

const products = generateProducts(datasetSizes.large);

// Build search instances for different algorithms
const algorithms = {
    levenshtein: new FuzzySearch<Product>({ keys: ["name"], algorithm: "levenshtein" }),
    damerauLevenshtein: new FuzzySearch<Product>({ keys: ["name"], algorithm: "damerau-levenshtein" }),
    smithWaterman: new FuzzySearch<Product>({ keys: ["name"], algorithm: "smith-waterman" }),
    jaroWinkler: new FuzzySearch<Product>({ keys: ["name"], algorithm: "jaro-winkler" }),
};
...
Deno.bench("Pattern: Exact match | Levenshtein", () => {
    algorithms.levenshtein.search("Apple Product 0", { threshold: 0.0, limit: 10 });
});

Deno.bench("Pattern: Exact match | Damerau-Levenshtein", () => {
    algorithms.damerauLevenshtein.search("Apple Product 0", { threshold: 0.0, limit: 10 });
});

// Pattern 2: Single insertion error
Deno.bench("Pattern: Insertion error | Levenshtein", () => {
    algorithms.levenshtein.search("Appple Product 0", { threshold: 0.3, limit: 10 });
});

// Pattern 3: Single deletion error
Deno.bench("Pattern: Deletion error | Levenshtein", () => {
    algorithms.levenshtein.search("Aple Product 0", { threshold: 0.3, limit: 10 });
});

// Pattern 4: Single substitution error
Deno.bench("Pattern: Substitution error | Levenshtein", () => {
    algorithms.levenshtein.search("Bpple Product 0", { threshold: 0.3, limit: 10 });
});

// Pattern 5: Transposition (Damerau-Levenshtein should be faster)
Deno.bench("Pattern: Transposition | Levenshtein", () => {
    algorithms.levenshtein.search("Aplpe Product 0", { threshold: 0.3, limit: 10 });
});

Deno.bench("Pattern: Transposition | Damerau-Levenshtein", () => {
    algorithms.damerauLevenshtein.search("Aplpe Product 0", { threshold: 0.3, limit: 10 });
});
...
Deno.bench("Pattern: 2 errors | Damerau-Levenshtein", () => {
    algorithms.damerauLevenshtein.search("Aple Prodct 0", { threshold: 0.4, limit: 10 });
});

// Pattern 9: Case sensitivity (should be handled by normalization)
Deno.bench("Pattern: Case insensitive | Levenshtein", () => {
    algorithms.levenshtein.search("apple product 0", { threshold: 0.3, limit: 10 });
});
...
const jaProducts: Product[] = [
    ...
    { id: "3", name: "オレンジ", category: "果物", description: "ジューシーなオレンジ" },
];

const jaSearch = new FuzzySearch<Product>({ keys: ["name", "category"], algorithm: "levenshtein" });
