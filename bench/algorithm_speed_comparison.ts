...
for (const { name } of sizes) {
  for (const algorithm of algorithms) {
    ...
    Deno.bench(`${algorithm.padEnd(20)} | ${name.padEnd(10)} | typo`, () => {
      search.search(queryPatterns.typo1char.product, {
        threshold: 0.3,
        limit: 10,
      });
    });
...
for (const { name } of sizes) {
  for (const algorithm of algorithms) {
    ...
    Deno.bench(`${algorithm.padEnd(20)} | ${name.padEnd(10)} | partial`, () => {
      search.search(queryPatterns.partial.product, {
        threshold: 0.4,
        limit: 10,
      });
    });
...
for (const { name } of sizes) {
  for (const algorithm of algorithms) {
    ...
    Deno.bench(
      `${algorithm.padEnd(20)} | ${name.padEnd(10)} | transposition`,
      () => {
        search.search(queryPatterns.transposition.product, {
          threshold: 0.3,
          limit: 10,
        });
      },
...
const searchForQueryLength = new FuzzySearch<Product>({
...
Deno.bench("Query length | long (25 chars)", () => {
  searchForQueryLength.search("High-quality electronics", {
    threshold: 0.4,
    limit: 10,
  });
});
